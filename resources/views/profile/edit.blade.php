    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <x-tabs :tabs="[
                ...
                <div x-show="tab === 'profile'" x-cloak>
                    <x-card-form title="Profile Information" description="Update your account's profile information, email, and profile picture.">
                        @if(session('status') === 'profile-updated')
                            <div class="text-green-600 mb-4 text-center">Profile updated successfully!</div>
                        @endif
                        ...
                        <form method="post" action="{{ route('profile.update') }}" class="space-y-6" enctype="multipart/form-data">
                            ...
                            <div class="flex flex-col items-center mb-6">
                                ...
                                <label class="mt-2 cursor-pointer text-indigo-600 hover:underline text-sm">
                                    ...
                                    <input type="file" name="profile_image" accept="image/*" class="hidden"
                                        @change="
                                            ...
                                            if(file) {
                                                ...
                                                reader.onload = e => { $refs.profileImage.src = e.target.result }
                ...
                <!-- Instructor Tab -->
                <div x-show="tab === 'instructor'" x-cloak>
                    @if($user->role === 'instructor')
                        <x-card-form title="My Courses" description="Here are the courses you are teaching.">
                            @if($user->courses && $user->courses->count())
                            ...
                            @else
                                <p class="text-gray-600 text-center">You have not created any courses yet.</p>
                            @endif
                        </x-card-form>
                    @else
                        <x-card-form title="Become an Instructor" description="You are not an instructor yet. Request to become an instructor to start creating courses." color="indigo-600">
                            <form method="post" action="{{ route('instructor.request') }}">
                                @csrf
                                <div class="flex justify-center mt-4">
                                    <x-primary-button>Request to be an Instructor</x-primary-button>
                                </div>
                            </form>
                        </x-card-form>
                    @endif
